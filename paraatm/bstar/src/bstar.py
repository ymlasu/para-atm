...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None):
        ...
        for i in range(self.num_layers):
            output, attn = self.layers[i](output, src_mask=mask, src_key_padding_mask=src_key_padding_mask)
            atts.append(attn)
        if self.norm:
...
class STAR(torch.nn.Module):
    ...
    def __init__(self, args, dropout_prob=0):
        ...
        # Decoder with Bayesian Liear Layer
        #self.decoder_layer = nn.Linear(32, 16)
        self.decoder_layer = BBBLinear(32, 16)  # Bayesian Linear Layer
        #self.output_layer = BBBLinear(16, 2)
    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum >= self.args.obs_length and iftest:
                ...
                node_abs_pred = shift_value[self.args.obs_length:framenum + 1, node_index] + \
                                outputs[self.args.obs_length - 1:framenum, node_index]
            ...
            #decoder_embedded = F.dropout(self.relu(self.decoder_layer(temporal_input_embedded)), p=0.1, training=True)

            # decoder
            #decoder_embedded = F.dropout(self.decoder_layer(temporal_input_embedded), p=0.1, training=True)
            decoder_embedded = self.decoder_layer(temporal_input_embedded)
            outputs_current = self.output_layer(decoder_embedded)
