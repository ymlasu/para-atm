...
class FlightPlanSelector:
	...
	def readFlightPlan(self, origin, destination):
		...
		tmp_Sids = self.natsSim.terminalAreaInterface.getAllSids(origin)
		...
		tmp_arrivalStars = self.natsSim.terminalAreaInterface.getAllStars(destination)
	...
	def generateDepartureTaxiPlan(self, origin_airport, departure_runway, origin_gate) :
		...
		if (tmp_node_seq == -1) :
			...
			tmp_array_node_map = self.natsSim.airportInterface.getLayout_node_map(origin_airport)
			...
			tmp_design_taxi_plan = self.natsSim.airportInterface.get_taxi_route_from_A_To_B("", origin_airport, origin_gate, departing_runway_entry)
	...
	def generateArrivalTaxiPlan(self, destination_airport, arrival_runway, destination_gate) :
		...
		tmp_array_node_data = self.natsSim.airportInterface.getLayout_node_data(destination_airport)
		if not(tmp_array_node_data is None) :
			for i in range(0, len(tmp_array_node_data)) :
		...
		if (tmp_node_seq == -1) :
			...
			tmp_array_node_map = self.natsSim.airportInterface.getLayout_node_map(destination_airport)
			if not(tmp_array_node_map is None) :
				for i in range(0, len(tmp_array_node_map)) :
					if (tmp_node_seq == tmp_array_node_map[i][1]) :
			...
			tmp_design_taxi_plan = self.natsSim.airportInterface.get_taxi_route_from_A_To_B("", destination_airport, landing_runway_end, destination_gate)
			if not(tmp_design_taxi_plan is None) :
				for i in range(0, len(tmp_design_taxi_plan)) :
					if not(i == 0) :
						tmp_surface_plan_string = tmp_surface_plan_string + ", "
