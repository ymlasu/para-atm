import paraatm
from paraatm.svm import SVM_ASRS_risk_category
import unittest
import numpy as np
import pandas as pd
import os
...
class TestSVM(unittest.TestCase):
    ...
    def test_SVM_input(self):
        ...
        y_train_path = os.path.join(THIS_DIR, '..', 'sample_data/SVM_classifier/y_train.csv')
        df2 = pd.read_csv(y_train_path)
        # Simple check:
        self.assertEqual(len(df2), 500)
        
        test_data_path = os.path.join(THIS_DIR, '..', 'sample_data/SVM_classifier/test_data.csv')
