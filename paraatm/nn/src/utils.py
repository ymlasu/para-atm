...
class Trajectory_Dataloader():
    ...
    def get_seq_from_index_balance(self, frameped_dict, pedtraject_dict, data_index, setname):
        ...
        for i in range(data_index.shape[1]):
            ...
            try:
                ...
                continue
            present_pedi = framestart_pedi | frameend_pedi  # get all ped ids exist from the startframe to the endframe
            ...
            for ped in present_pedi:
                ...
                cur_trajec = (cur_trajec[:, 1:].reshape(-1, 1, 2),)  # modify this if 3d
    ...
    def get_social_inputs_numpy(self, inputnodes):
        ...
        seq_list = np.zeros((inputnodes.shape[0], num_Peds))  # which ped exist in the current timeframe, a 0-1 binary array

        # denote where data not missing
        for pedi in range(num_Peds):
            seq = inputnodes[:, pedi]  # trajectory for each ped
            seq_list[seq[:, 0] != 0, pedi] = 1  # exist location in ped
        ...
        for pedi in range(num_Peds):
            ...
            for pedj in range(num_Peds):
                ...
                # if seqi[select, -1] - seqi[select, -1] < 1000:

                # else:
                select = (seq_list[:, pedi] > 0) & (seq_list[:, pedj] > 0)

                # calculate the haversine distance between two trajectory sequences
                hdist = vectorized_haversine_dist(seqi[select, 1], seqi[select, 0], seqj[select, 1], seqj[select, 0])
                select_dist = (hdist > self.args.neighbor_thred)

                #relative_cord = seqi[select, :] - seqj[select, :]  # relative coords

                # invalid data index
                # change threshold here
                # change to L2 norm
                #select_dist = np.sqrt(relative_cord[:, 0]**2 + relative_cord[:, 1]**2) > self.args.neighbor_thred
